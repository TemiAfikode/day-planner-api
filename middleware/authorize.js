...
module.exports = async function (req, res, next) {
    try {
        ...
        if (!authToken) return res.status(401).send({ isSuccessful: false, message: 'Access denied, login required' });
        ...
        if (!token) {
            return res.status(401).send({ isSuccessful:false, message: 'Access denied, login required' });
        }
